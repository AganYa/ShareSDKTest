...
- (void)viewDidLoad {
  ...
  //添加测试按钮
  [self addButtonWithName:NSLocalizedString(@"CustomizeShareEditView", nil)
                   action:@selector(customizeShareEditView:)
                    index:0];
  [self addButtonWithName:NSLocalizedString(@"CallOpenAPI", nil)
                   action:@selector(callOpenAPI:)
                    index:1];
}
...
- (void)addButtonWithName:(NSString *)buttonName
  ...
  [btn sizeToFit];
  btn.frame = CGRectMake(self.view.frame.size.width / 2 - 150, 50 + index * 50,
                         300, 40);
  [btn addTarget:self
                action:action
...
- (void)customizeShareEditView:(id)sender {
  id<ISSContent> publishContent =
      [ShareSDK content:@"Share Content"
          defaultContent:@"Default Content"
                   image:[ShareSDK imageWithUrl:@"http://www.mob.com/mob/"
                                                @"images/index/sharesdk1.png"]
                   title:@"title"
  ...
  id<ISSShareOptions> shareOptions = [ShareSDK
      defaultShareOptionsWithTitle:@"Title"
                   oneKeyShareList:[ShareSDK
                                       getShareListWithType:ShareTypeSinaWeibo,
                                                            nil]
                    qqButtonHidden:YES
               ...
               shareOptions:shareOptions
                     result:^(ShareType type, SSResponseState state,
                       ...
                       if (state == SSPublishContentStateSuccess) {
                         ...
                         NSLog(@"Failed,error code == %zi, error code == %@",
...
- (void)viewOnWillDisplay:(UIViewController *)viewController
  ...
  [leftBtn1 setBackgroundImage:[UIImage imageNamed:@"ShareButtonBG.png"]
                      forState:UIControlStateNormal
                    barMetrics:UIBarMetricsDefault];
  [rightBtn1 setBackgroundImage:[UIImage imageNamed:@"ShareButtonBG.png"]
                       forState:UIControlStateNormal
                     barMetrics:UIBarMetricsDefault];
}
...
- (void)callOpenAPI:(id)sender {
  if ([ShareSDK hasAuthorizedWithType:ShareTypeSinaWeibo]) {
    ...
    [ShareSDK
        authWithType:ShareTypeSinaWeibo
             options:nil
...
- (void)callOpenAPIAfterAuthorize {
  ...
  id<ISSSinaWeiboApp> app =
      (id<ISSSinaWeiboApp>)[ShareSDK getClientWithType:ShareTypeSinaWeibo];
      ...
      fault:^(CMErrorInfo *error) {
        UIAlertView *alertView = [[UIAlertView alloc]
                initWithTitle:@"Failed"
                      message:[NSString
                                  stringWithFormat:
                                      @"Error Code：%zi, Error Description:%@",
                                      [error errorCode],
                                      [error errorDescription]]
                     delegate:nil
            cancelButtonTitle:@"OK"
